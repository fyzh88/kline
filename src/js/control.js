...
export class Control {
    ...
    static requestSuccessHandler(res) {
        ...
        if (!res || !res.success) {//返回的数据不正确或者没有返回数据，间歇性请求数据
        ...
        //更新K线数据
        let updateDataRes = Kline.instance.chartMgr.updateData("frame0.k0", Kline.instance.data.lines);//这里的返回值是关键
    ...
    static setHttpRequestParam(symbol, range, limit, since) {
        let str = "symbol=" + symbol + "&range=" + range;/*range代表的是时间周期*/
        //limit和since二选一
        if (limit !== null)
            str += "&limit=" + limit;
        else
