...
export class ChartManager {
    ...
    clearDataSource(dsName,dsAlias){
        let cached = this.getCachedDataSource(dsAlias);
        if (cached !== undefined && cached !== null) {
            this.setDataSource(dsName,null,true)
            this.setCachedDataSource(dsAlias,null)
        }
    }
    ...
    //ajax请求完成数据之后，将拿到的数据给到这里，进行数据渲染
    updateData(dsName, data) {/*dsName=frame0.k0 data=lines数据*/
        let ds = this.getDataSource(dsName);
        if (ds === undefined || ds === null) {
            return;
        }

        if (data !== undefined && data !== null) {//传进来的数据有效
            if (!ds.update(data)) {//将传进来的数据data更新到ds中
                return false;
            }
            //更新完了之后发现无需刷新界面
